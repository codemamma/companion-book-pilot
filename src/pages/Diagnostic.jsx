...
function Diagnostic() {
  ...
  const calculateScareScore = (responses) => {
    const total = Object.values(responses).reduce((sum, val) => sum + val, 0);
    const index = Math.round(((total - 5) / 15) * 100);

    const dimensions = {};
    scareQuestions.forEach((q) => {
      if (responses[q.id]) {
        dimensions[q.dimension] = responses[q.id];
      }
    });
  ...
  const detectFocusArea = (dimensions) => {
    ...
    return dimensionToArea[topDimension] || 'communicate';
  };
  ...
  const handleScareSubmit = () => {
    const { total, index, dimensions } = calculateScareScore(scareResponses);
    const area = detectFocusArea(dimensions);
    setFocusArea(area);
    setStage('score');
  };
  ...
  const handleFocusedNext = () => {
    const questions = focusedAssessments[focusArea];
            ...
            {scareQuestions.map((question, index) => (
              <div key={question.id} className="bg-gray-900 border border-gray-800 rounded-lg p-6">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <h3 className="text-lg font-medium text-white pt-1">{question.question}</h3>
                </div>
  ...
  const renderFocus = () => {
    const area = caresAreas[focusArea];
  ...
  const renderRecommendations = () => {
    const area = caresAreas[focusArea];
    const recommendations = chapterRecommendations[focusArea];
    const profile = getProfileData();
      ...
      <section className="py-20 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-8 text-center">
            <h2 className="text-2xl font-bold text-white mb-4">Want More Support?</h2>
            <p className="text-gray-400 mb-6">
              Explore the full CARES framework, use the AI coach, or connect with Saby for personalized guidance.
            </p>
